...
// Simple MD5 implementation for signatures
...
export class LibreFmScrobbler {
  ...
  private readonly API_URL = "https://libre.fm/2.0/";
  ...
  private async loadSession() {
    const session = await storageService.getItem("librefm_session");
    if (session) {
  ...
  async setSession(key: string, username: string) {
    this.sessionKey = key;
    await storageService.setItem("librefm_session", { key, username });
  }
  ...
  private async makeRequest(method: string, params: Record<string, any> = {}, requiresAuth = false) {
    ...
    try {
      ...
      console.error("Libre.fm request failed:", e);
      throw e;
    }
  }
  ...
  private async scrobble() {
    ...
    try {
      ...
      console.log("[Libre.fm Scrobbler] Scrobbled:", this.currentTrack.title);
    } catch (e) {}
  }
